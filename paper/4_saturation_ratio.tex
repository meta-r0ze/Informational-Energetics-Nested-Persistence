...
This derivation demonstrates that the Casimir invariants of the Standard Model are not fundamental inputs, but are the \textbf{algebraic projections} of the underlying lattice topology. The successful prediction of QED screening ($4/3$) from identical geometric principles confirms this is a fundamental property of embedding topological charges in 4-dimensional spacetime.
...
The $E_8$-Persistence theory resolves the Strong CP problem not by dynamic suppression, but by \textbf{Topological Prohibition}. The interaction subspace that hosts the Strong Force ($\sigma-\chi=3$) is geometrically orthogonal to the spacetime boundary ($\chi=2$) where topological winding numbers are defined. This prohibition is enforced by two independent and mutually reinforcing information-theoretic principles:
