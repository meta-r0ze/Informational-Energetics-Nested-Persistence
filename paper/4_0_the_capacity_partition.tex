...
\begin{enumerate}
    \item \textbf{Capacity: The Chiral Bandwidth} ($\nu = 16$). The $E_8$ substrate possesses 16 chiral degrees of freedom. This acts as the absolute processing budget that must be distributed across all tasks without overflow. Because the partition requires a shared coordinate grid, this budget is subject to the Coordinate Overhead ($\eta = 171/172$) whenever signals traverse the bulk manifold. As every channel is active the impedance load becomes $\alpha_s$.
