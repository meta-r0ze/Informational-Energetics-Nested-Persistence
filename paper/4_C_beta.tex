...
\begin{enumerate}
    ...
    \item \textbf{The Sterile Channel ($\nu_R$):} The $E_8 \to D_4 \oplus D_4$ decomposition assigns the 16 physical spinors to the $8_s$ representation of each $D_4$ factor. $D_4$ triality is the $S_3$ symmetry permuting the three inequivalent 8-dimensional representations $\{8_v, 8_s, 8_c\}$. Under the $SU(2)$ action of the topological boundary $\chi=2$ (the weak isospin generator), the $8_s$ decomposes into $SU(2)$ doublets plus exactly one $SU(2)$ singlet. This singlet is triality-forced: it corresponds to the direction in $8_s$ that maps to $8_c$ under the triality automorphism and therefore carries zero weak isospin --- it is the unique $D_4$ representation fixed by the triality action relative to the $SU(2)$ boundary. Triality does not merely \emph{identify} this channel after the fact; it \emph{guarantees} exactly one such singlet exists among the 16 chiral states (full derivation deferred to the companion paper on fermion structure). This singlet is the \textbf{geometric vacancy}: a topological absence, not a propagating particle. At full saturation, the unitarity constraint locks exactly this channel. The effective active capacity is therefore $(\nu-1)/\nu = 15/16$.
\end{enumerate}

\textit{Why multiplicative?} In Shannon's channel capacity model, a coupling constant represents the fraction of the substrate's transmission probability allocated to a given interaction. If one of $\nu$ independent channels is blocked (locked as a reference state), the remaining $(\nu-1)$ channels transmit freely, but the total available probability space has not changed --- it is still normalized to $\nu$ channels. The coupling therefore scales as the ratio of active-to-total probability: $(\nu-1)/\nu$. An additive correction would imply the blocked channel contributes negative coupling, which has no geometric meaning; the multiplicative ratio correctly reflects the fractional reduction in available transmission probability.
...
\subsubsection{Derivation 3: Geometric Universality --- The QED Beta Coefficient}
...
\subsubsection{Consistency Check: The Z-Pole Running Coupling (Empirical Anchor)}
